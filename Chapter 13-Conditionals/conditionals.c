...
void lval_print(lval *v) {
    switch (v->type) {
        ...
        case LVAL_NUM: printf("%li", v->num);
            break;
        case LVAL_BOOL: if (v->bool == 1) printf("true");
            if (v->bool == 0) printf("false");
            break;
...
lval *builtin_ord(lenv *e, lval *a, char *op) {
    ...
    if (strcmp(op, "!") == 0) {
        ...
        if (a->cell[0]->type == LVAL_NUM)
            ...
            r = !a->cell[0]->bool;
        lval_del(a);
        return r == 1 ? lval_bool("true") : lval_bool("false");
    }
    LASSERT_NUM(op, a, 2);
    ...
    if (a->cell[0]->type == LVAL_BOOL && a->cell[1]->type == LVAL_BOOL) {
        if (strcmp(op, ">") == 0) {
    ...
    lval_del(a);
    if (r == 1)return lval_bool("true");
    if (r == 0)return lval_bool("false");
}
...
int main(int argc, char **argv) {
    ...
    mpc_cleanup(7, Number, Symbol, Sexpr, Qexpr, Expr, Lispy, Bool);
